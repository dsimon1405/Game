...
G_GUI_W_RestartLevel::G_GUI_W_RestartLevel()
    ...
    bt_Go_to_main_menu({ &G_GUI_W_RestartLevel::CallMainMenu, this }, {}, true,
        { G_LangText{ .lang = G_L_Russian, .text = L"В главное меню" }, { .lang = G_L_English, .text = L"Go to main menu" } }),
    bt_restart_level({ &G_GUI_W_RestartLevel::RestartLevel, this }, {}, true,
        { G_LangText{ .lang = G_L_Russian, .text = L"Уровень заново" }, { .lang = G_L_English, .text = L"Level again" } })
{
    ...
    this->AddRow(ZC_GUI_Row(ZC_GUI_RowParams(0.f, ZC_GUI_RowParams::Center, 30.f, 20.f), { bt_Go_to_main_menu.GetObj(), bt_restart_level.GetObj() }));
}
